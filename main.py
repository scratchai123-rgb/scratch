...
# MARKDOWN STRIPPER - only removes formatting, keeps normal symbols
def strip_markdown(text):
    text = re.sub(r'^#{1,6}\s+', '', text, flags=re.MULTILINE)  # headings at start of line only
    ...
    text = re.sub(r'^\s*[-*+]\s+', '', text, flags=re.MULTILINE) # bullet points at line start
...
def chat(message):
    ...
    try:
        response = client.messages.create(
            ...
            max_tokens=200,
            system="You are a helpful assistant. Keep all your responses to 200 characters or less. Be concise and clear. Do not use markdown formatting like headings, bullet points, or bold text.",
